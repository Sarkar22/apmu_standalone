...
module axi_lite_regs #(
  ...
  // // Cycle-aligned monitor: show per-cycle AW/W/B handshake status
  // always_ff @(posedge clk_i) begin
  //   if (!rst_ni) begin
  //     // reset
  //   end else begin
  //     if (axi_req_i.aw_valid || axi_req_i.w_valid || axi_resp_o.b_valid) begin
  //       $display("TIME=%0t AXI_LITE_REGS: aw_valid=%b aw_addr=0x%08h aw_prot_ok=%b aw_ready=%b | w_valid=%b w_data=0x%08h w_ready=%b | b_valid=%b b_ready=%b b_resp=%0d chunk_loaded=%b",
  //                $time,
  //                axi_req_i.aw_valid,
  //                axi_req_i.aw.addr,
  //                aw_prot_ok,
